...
exports.default = function () {
  ...
  var wbItemWrap = new CreateWbItemWrap({ onDrawChange: onDrawChange, role: role }, target);
...
function CreateWbItemWrap(_ref4, target) {
  ...
  return {
    ...
    handleShow: function handleShow(path, callback) {
      ...
      group.data('origTransform', path.transform().local).attr({ x: x, y: y, width: width, height: height }).wBtoFront(group);
      square2.attr({
        x: x - 10,
        y: y - 10,
        width: width + 20,
