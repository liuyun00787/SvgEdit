...
exports.default = function (_ref) {
  var _ref$role = _ref.role,
      ...
      _ref$pptConfig = _ref.pptConfig,
      pptConfig = _ref$pptConfig === undefined ? {} : _ref$pptConfig;
  ...
  var goTo = function goTo(page) {
    setState({ page: page }, function () {
      if (group.selectAll('.page')) {
        group.selectAll('.page').animate({ opacity: 0 }, 500);
        group.select('.page-' + state.page).animate({ opacity: 1 }, 500).attr({ opacity: 1 }).PageToFront();
        ...
        if (globalPlayer) {
          ...
          if (group.select('.page-' + state.page).attr('__PAGETYPE__') === 'video') {
            var _ref4 = group.select('.page-' + state.page).data('info') || {},
                _ref4$content = _ref4.content,
                content = _ref4$content === undefined ? [] : _ref4$content;
